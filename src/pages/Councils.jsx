...
function CouncilCard({ council, onOpen }) {
  return (
    <Card className="border-border shadow-sm overflow-hidden hover:shadow-md transition-shadow">
      ...
      <div className="p-4 sm:p-5 space-y-4">
        ...
        <div className="flex items-center gap-4 text-sm flex-wrap">
          <div className="flex items-center gap-1.5 text-muted-foreground">
            ...
            <span style={{ fontVariantNumeric: "tabular-nums" }}>{council.acceptedMembers}/{council.maxMembersAllowed} thành viên</span>
          </div>
          <div className="flex items-center gap-1.5 text-muted-foreground">
        ...
        {council.decision && (
          <div className="bg-green-50 border border-green-200 rounded-lg p-3">
            ...
            {council.decision.averageScore && (
              <p className="text-xs text-green-600 mt-1 font-medium" style={{ fontVariantNumeric: "tabular-nums" }}>Điểm trung bình: {council.decision.averageScore}/100</p>
            )}
          </div>
        )}
      </div>
      ...
      {/* Card Footer */}
      <div className="px-4 sm:px-5 py-3 border-t border-border bg-surface/30 flex justify-end">
        <Button variant="ghost" size="sm" className="text-primary" onClick={() => onOpen(council)}>
          <Eye className="h-4 w-4 mr-1" aria-hidden="true" /> Xem chi tiết <ChevronRight className="h-4 w-4 ml-1" aria-hidden="true" />
        </Button>
      </div>
    </Card>
  );
...
function CouncilDetailDialog({ open, onOpenChange, council }) {
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-6 mt-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            ...
            <InfoBlock label="QĐ thành lập" value={council.establishmentDecisionNo} />
...
export default function CouncilsPage() {
  ...
  if (loading) {
    return <LoadingState message="Đang tải hội đồng…" />;
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {councils.length === 0 ? (
          <div className="col-span-full">
            <EmptyState
              icon={Users}
              title="Bạn chưa tham gia hội đồng nào"
              description="Khi được mời tham gia hội đồng xét duyệt, thông tin sẽ hiển thị tại đây."
            />
          </div>
        ) : (
          councils.map((council) => (
            <CouncilCard key={council.id} council={council} onOpen={openDetail} />
          ))
        )}
      </div>
