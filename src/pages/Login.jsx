...
export default function LoginPage() {
  ...
  return (
    <div className="flex flex-col md:flex-row w-full min-h-screen bg-background text-foreground font-sans antialiased">
      {/* Left Pane - Branding */}
      <div className="w-full md:w-1/2 flex flex-col justify-center items-center p-10 relative z-10 text-center md:text-left bg-gradient-to-br from-tertiary-container/30 to-primary-container/20 overflow-hidden">
        <div className="max-w-md w-full relative z-20 flex flex-col gap-6">
          <h1 className="text-5xl font-bold text-primary tracking-tight">FURPMS</h1>
          <h2 className="text-2xl md:text-3xl font-semibold text-secondary-foreground">
            FPT University Research Project Management System
          </h2>
          <p className="text-lg text-muted-foreground mt-6 border-l-4 border-primary pl-4">
            Empowering academic excellence and research innovation.
          </p>
        </div>
        {/* Decorative elements */}
        <div className="absolute top-1/4 right-1/4 w-64 h-64 bg-primary/10 rounded-full blur-3xl pointer-events-none z-0"></div>
        <div className="absolute bottom-1/4 left-1/4 w-96 h-96 bg-tertiary/10 rounded-full blur-3xl pointer-events-none z-0"></div>
      </div>

      {/* Right Pane - Login Container */}
      <div className="w-full md:w-1/2 bg-surface flex items-center justify-center p-10 relative z-20">
        <div className="w-full max-w-md bg-card/80 backdrop-blur-md border border-border rounded-xl shadow-md p-10 flex flex-col gap-8">
          <div className="text-center md:text-left">
            <h2 className="text-3xl font-semibold text-card-foreground mb-2">Welcome Back</h2>
          ...
          <div className="flex flex-col gap-6">
            <Button
              ...
              disabled={isLoading}
              className="w-full bg-primary hover:bg-primary/90 text-primary-foreground py-6 rounded-lg text-lg flex items-center gap-3 transition-all"
            >
              <LoginIcon className="w-5 h-5" /> Login with FPT SSO
            </Button>
          </div>
